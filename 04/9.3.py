...
class ATM:
    def __init__(self):
        ...
        try:
            with open('operations_put_get.txt', 'rb') as f:
                self.operations_list = pickle.load(f)
        except FileNotFoundError:
            self.operations_list = []

    # пишем дамп списка в файл
    def __write_dump(self):
        pickle.dump(self.operations_list, open('operations_put_get.txt', 'wb'), protocol=pickle.HIGHEST_PROTOCOL)
    ...
    def put_to_account(self):
        ...
        if amount.isdigit() and int(amount) % 50 == 0:
            ...
            self.__write_to_acc()
            self.operations_list.append(f'+{amount}')
            self.__write_dump()
...
atm1 = ATM()
while True:
    print(f'Записей в листе: {atm1.operations_list = }')
